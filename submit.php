<?php  
	...
	if(isset($_GET['content']) || isset($_GET['from']) || isset($_GET['to'])){
		if (strpos($_GET['content'], '\\') || strpos($_GET['from'], '\\') || strpos($_GET['to'], '\\'))
		{
	...
	if($content!=""){
		...
		// judge repeat
		$sql = "SELECT fromWho, toWho, content FROM wishwall_love ORDER BY ID DESC LIMIT 0,10;";
		$result = mysql_query( $sql, $conn );
		$p = true;
		while ($row = mysql_fetch_assoc($result))
			if ( ($row['content'] == $content) && ($row['fromWho'] == $from) && ($row['toWho'] == $to) ) $p = false;
		if ($p)
		{
